...
export const ProjectList = ({ favoritesOnly = false, limit = 10 }) => {
    ...
    const { projects, loading, hasNextPage, loadMore } = useProjects({
        favoritesOnly,
        limit,
    });
    ...
    return loading && projects.length === 0 ? (
        <div
            className={`grid grid-cols-1 lg:grid-cols-2 gap-2 space-y-2 py-4 mx-2 space-x-2 grid-flow-row-dense text-secondary`}
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 space-y-2 py-4 mx-2 space-x-2 grid-flow-row-dense">
            ...
            <div
                className={`w-full flex items-center lg:col-span-2 justify-center text-secondary ${loading ? "visible" : "invisible"}`}
            >
                <Loading />
            </div>
