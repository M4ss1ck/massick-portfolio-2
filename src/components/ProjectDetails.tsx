...
export const ProjectDetails = ({ id }: { id: string | number }) => {
    ...
    return (
        <div className='grid grid-cols-1 gap-2 space-y-2 py-4 mx-2 space-x-2 grid-flow-row-dense max-w-prose'>
            ...
            {project.url && <p className='text-other'><Link className='underline-animation' href={project.url}>URL</Link></p>}
            {project.demo && <p className='text-other'><Link className='underline-animation' href={project.demo}>DEMO</Link></p>}
            ...
            <h2 className='text-xl text-other font-display'>{t("Related Projects")}</h2>
